...
public class Window extends JFrame{
	...
	private Color DARK_GREEN = new Color(0, 127, 0);
	...
	private void addComponents(){
		...
		pConsole.add(areaConsole,BorderLayout.CENTER);
		...
		lClients.addMouseListener(new MouseListener() {
			...
			public void mouseClicked(MouseEvent e) {
				if(e.getClickCount() == 2){
					...
					addText(Color.GRAY, "SERVER", server.getUser(SelectedID).toString());
	...
	public void addText(Color c,String von,String text){
	    ...
	    try {
	    	areaConsole.getDocument().insertString(areaConsole.getDocument().getLength(), text + "\n",colors.get(Color.black) );
		} catch (BadLocationException e) {
			e.printStackTrace();
		}
	}
